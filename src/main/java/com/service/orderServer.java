...
*/	       
	...
	@Transactional(rollbackFor = Exception.class)
    public void processNotify(Map<String, String> params,String cart) throws Exception {
        ...
        parentOrder order = parent.getOrderLock(Integer.valueOf(orderId));//这里用悲观锁金额操作防止并发 
        if (null == order) {
            throw new BigException("订单不存在");
        }

        if (totalAmount != order.getTotal()) {
        ...
        //修改订单状态
        int rows = parent.updateOrderStatus(order.getId(), OrderStatusEnum.un_received.getK());//这里是支付页面向结果页面此时已经支付,根据支付结果更改状态 
        if (1 != rows) {
    ...
    public ArrayList<orderDTO> order(int userId) throws Exception{
